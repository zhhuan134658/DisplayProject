/* * @Author: NorthWind * @Date: 2021-03-18 09:00:52 * @Last Modified by:
NorthWind * @Last Modified time: 2022-01-13 14:31:47 */
<template>
  <div id="workManageFinish">
    <!-- 劳务完工结算列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          :isFooter="isFooter"
          @listenChild="getMult"
          printTableName="service_warranty_settlement"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  ...
  methods: {
    ...
    getTitleList() {
      this.$axios
        .post('/spread/showFiledCurrencyList', {
          name: 'service_warranty_settlement',
        })
        .then(res => {
          if (res.data.code == 1) {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/projecttwo/serviceWarrantySettlement')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    backList() {
      this.$axios
        .post('/projecttwo/serviceWarrantySettlementReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/projecttwo/serviceWarrantySettlementDownload', {
