/* * @Author: NorthWind * @Date: 2020-12-09 09:28:34 * @Last Modified by:
NorthWind * @Last Modified time: 2022-01-13 11:39:55 */
<template>
  <div id="workProgressMoney">
    <!-- 劳务进度款列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          :isFooter="isFooter"
          @listenChild="getMult"
          printTableName="service_progresspay_declaration"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  ...
  methods: {
    ...
    getTitleList() {
      this.$axios
        .post('/spread/showFiledCurrencyList', {
          name: 'service_progresspay_declaration',
        })
        .then(res => {
          if (res.data.code == 1) {
    ...
    backList() {
      this.$axios
        .post('/project/serviceProgresspayDeclarationReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/project/serviceProgresspayDeclarationDownload', {
