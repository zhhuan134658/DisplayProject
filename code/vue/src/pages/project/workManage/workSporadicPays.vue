/* * @Author: NorthWind * @Date: 2021-04-14 10:59:57 * @Last Modified by:
NorthWind * @Last Modified time: 2022-01-13 14:36:22 */
<template>
  <div id="workSporadic">
    <!-- 零星劳务列表 -->
    <div class="main">
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          @creation="creation"
            :types='types'
            @getlist='getList'
          printTableName="sporadic_service_payment"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  name: 'workSporadicPay',
  components: { draggable, commonTable, commonSearch },
  data() {
    return {
        types:'sporadic_service_payment',
              payVisible: false,
     payformInline: {
        accountname: '',
        newaccountname: '',
  ...
  methods: {
          ...
          .then(() => {
            this.$axios
              .post('/newtao/payOperation', {
                ...
                id: val.id,
                pay_status:'1',
                biao_name:'sporadic_service_payment'
              })
              .then(res => {
                if (res.data.code == 1) {
                  this.getList();
    ...
    getList(ntype) {
      this.$axios
        .post('/finance/sporadiclist', {
          ...
          projec_tname: this.formInline.project_name,
    ...
    backList() {
      this.$axios
        .post('/project/sporadicServiceListReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
