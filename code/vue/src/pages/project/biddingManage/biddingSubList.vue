/* * @Author: NorthWind * @Date: 2020-12-08 08:58:37 * @Last Modified by:
mikey.zhaopeng * @Last Modified time: 2022-01-10 15:06:35 */
<template>
  <div id="biddingSubList">
    <!-- 分包物资招标列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          :isFooter="isFooter"
          @listenChild="getMult"
          printTableName="subcontract_bidding_plan"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  ...
  methods: {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/project/subcontractBiddingPlan')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    backList() {
      this.$axios
        .post('/project/subcontractBiddingPlanReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/project/subcontractBiddingPlanDownload', {
