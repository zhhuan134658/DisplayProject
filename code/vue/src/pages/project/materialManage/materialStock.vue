/* * @Author: NorthWind * @Date: 2020-12-08 15:23:43 * @Last Modified by:
NorthWind * @Last Modified time: 2022-01-13 11:38:58 */
<template>
    ...
    <div class="main">
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          printTableName="inventory_beginning"
        ></commonTable>
        <div class="page">
          <el-pagination
      ...
      >
        ...
        >
          ...
          <el-form-item label="库房：" prop="ck_id">
            <el-select
              v-model="upForm.ck_id"
              placeholder="请选择库房"
              filterable
              @change="ckSearch"
            >
...
export default {
  name: 'materialStock',
  components: { eupload, draggable, commonTable, commonSearch },
  data() {
    return {
      loading: false,
      //搜索条件
  ...
  methods: {
    ...
    ckSearch(val) {
      let obj = {};
      obj = this.cangList.find(item => {
        return item.id === val;
      });
      this.upForm.cang_ku = obj.name;
    },
    getMult(val) {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/project/inventoryBeginning')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    getList(ntype) {
      this.$axios
        .post('/project/inventoryBeginningList', {
          ...
          wh_person: this.formInline.wh_person,
    ...
    backList() {
      this.$axios
        .post('/project/inventoryBeginningReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/project/inventoryBeginningDownload', {
    ...
    derive() {
      dd.ready(function () {
        dd.biz.util.downloadFile({
          url: 'http://dingzhukuai.zzdingyun.com/public/file/库存期初模板.xls',
          name: '库存期初模板.xls', //定义下载文件名字
          onProgress: function (msg) {
            // 文件下载进度回调
          },
          onSuccess: function (result) {},
    ...
    upSave(formName) {
      ...
      if (_this.upForm.url) {
        _this.$refs[formName].validate(valid => {
          if (valid) {
            _this.$axios
              .post('/projectone/materialStockImport', _this.upForm)
              .then(res => {
                if (res.data.code == 1) {
                  _this.loading = false;
                  _this.getList();
