/*
 * @Author: NorthWind 
 * @Date: 2021-01-19 15:46:15 
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2022-01-21 17:06:32
 */
<template>
        ...
        >
          ...
          >
            <template slot="header" slot-scope="scope">
              <el-tooltip placement="top">
                <div slot="content">来源于：材料总计划明细的“总计划量”合计</div>
                <div>需求数量</div>
              </el-tooltip>
            </template>
          </el-table-column>
          ...
          >
            <template slot="header" slot-scope="scope">
              <el-tooltip placement="top">
                <div slot="content">
                  来源于：材料需用计划明细的“需用数量”合计
                </div>
                <div>已提计划量</div>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column align="left" :show-overflow-tooltip="true" prop="sy">
            <template slot="header" slot-scope="scope">
              <el-tooltip placement="top">
                <div slot="content">=总计划数量-已提计划量</div>
                <div>剩余量</div>
              </el-tooltip>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios
        .post('/projectone/materialPlanImplement', {
        ...
        .then(res => {
          if (res.data.code == 1) {
            ...
            this.yxiaoji = res.data.need_quantity_subtotal;
            this.yheji = res.data.need_quantity_total;
            this.qxiaoji = res.data.zh_plan_quantity_subtotal;
            this.qheji = res.data.zh_plan_quantity_total;
            this.sxiaoji = res.data.sy_subtotal;
            this.sheji = res.data.sy_total;
            this.$nextTick(() => {
              setTimeout(() => {
                this.adjustWidth();
              }, 500);
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/projectone/materialPlanImplementDownload', {
