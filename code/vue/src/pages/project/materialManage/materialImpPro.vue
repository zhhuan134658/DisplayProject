 ...
 * @Last Modified time: 2022-08-16 15:45:12
 */
<template>
...
export default {
  ...
  methods: {
    tableRowClass({ row, column, rowIndex, columnIndex }) {
      let newArr = ['zh_plan_quantity', 'rk_number', 'material_need_quantity'];
    ...
    viewDia(row, column, cell, event) {
      ...
      } else if (column.property == 'material_need_quantity') {
        //申请量
        this.titleData = '申请量数据来源';

        this.proName = `${'项目名称:' + row.project_name}`;
        this.totalMoney = `${'申请量:' + row.material_need_quantity}`;
