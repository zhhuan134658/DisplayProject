/*
 * @Author: NorthWind 
 * @Date: 2020-12-12 10:58:56 
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2022-01-21 17:07:45
 */
<template>
  <div id="progressBuild">
    <!-- 设备计划列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        <el-row class="maincBtn mainMoveBtn">
          ...
          <div class="mmbRight" v-if="$store.state.userInfo.admin == 1">
            <i class="el-icon-s-tools"></i>
            <el-button type="text" @click="viewShow">设置显示字段</el-button>
          </div>
        </el-row>
        <commonTable
          :tpList="tpList"
...
export default {
  ...
  methods: {
    ...
    getTitleList() {
      this.$axios
        .post('/spread/showFiledCurrencyList', {
          name: 'construction_progress_plan',
        })
        .then(res => {
          if (res.data.code == 1) {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/project/constructionProgressPlan')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    getList(ntype) {
      this.$axios
        .post('/project/constructionProgressPlanList', {
          ...
          main_task: this.formInline.main_task,
          sub_task: this.formInline.sub_task,
    ...
    backList() {
      this.$axios
        .post('/project/constructionProgressPlanReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/project/constructionProgressPlanDownload', {
