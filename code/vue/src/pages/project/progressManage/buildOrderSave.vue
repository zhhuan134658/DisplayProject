/*
 * @Author: NorthWind 
 * @Date: 2020-12-12 11:22:41 
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2022-01-21 17:07:27
 */
<template>
  <div id="buildOrderSave">
    <!-- 施工维护列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
...
export default {
  ...
  methods: {
    ...
    getTitleList() {
      this.$axios
        .post('/spread/showFiledCurrencyList', {
          name: 'construction_list_maintenance',
        })
        .then(res => {
          if (res.data.code == 1) {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/project/constructionListMaintenance')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    getList(ntype) {
      this.$axios
        .post('/project/constructionListMaintenanceList', {
          ...
          maintainer: this.formInline.maintainer,
    ...
    backList() {
      this.$axios
        .post('/project/constructionListMaintenanceReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/project/constructionListMaintenanceDownload', {
