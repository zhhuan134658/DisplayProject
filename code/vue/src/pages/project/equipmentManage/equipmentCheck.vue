/* * @Author: NorthWind * @Date: 2020-12-10 14:25:10 * @Last Modified by:
NorthWind * @Last Modified time: 2022-01-13 14:00:09 */
<template>
  <div id="equipmentCheck">
    <!-- 设备检查列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          :isFooter="isFooter"
          @listenChild="getMult"
          printTableName="equipment_inspection"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  ...
  methods: {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/project/equipmentInspection')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    backList() {
      this.$axios
        .post('/project/equipmentInspectionReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/project/equipmentInspectionDownload', {
