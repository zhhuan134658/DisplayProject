/* * @Author: NorthWind * @Date: 2020-11-26 11:19:59 * @Last Modified by:
mikey.zhaopeng * @Last Modified time: 2021-12-30 15:19:58 */
<template>
  <div id="tenderFile">
    <!-- 投标文件列表 -->
    <div class="main">
    ...
    <el-tab-pane label="投标支出申请" name="first"></el-tab-pane>
    <el-tab-pane label="招标文件购买申请" name="second" v-if="iscorpcreated"></el-tab-pane>
  </el-tabs>
      </div>
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        <el-row class="maincBtn mainMoveBtn" v-if="activeName=='first'">
          <div class="mmbLeft">
        ...
        <commonTable
          ...
          @getlist="getList"
           :isSet='false'
            :isdele8="true"
            types='bid_expenditure_apply'
          :printTableName="'bid_expenditure_apply'"
        ></commonTable>
...
export default {
  ...
  methods: {
    ...
    getMult(val) {
      ...
      this.$axios
        .post('/spread/showFiledList', {
          name: 'bidding_file_buy_spreadfield',
        })
        .then(res => {
          if (res.data.code == 1) {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/projecttwo/bidExpenditureApplyAdd')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    getList(ntype) {
      this.$axios
        .post('/projecttwo/bidExpenditureApplyList', {
          page: this.currentPage,
          number: this.pagesize,
          start_riqi: this.formInline.startTime,
          end_riqi: this.formInline.endTime,
    ...
    deleteList() {
      if (this.multSelectData.length > 0) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          .then(() => {
            this.$axios
              .post('/projecttwo/bidExpenditureApplyDel', {
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/projecttwo/bidExpenditureApplyload', {
  ...
  created() {
    ...
    this.getTitleList();
    this.getList();
       console.log(Number(this.$store.state.userInfo.corpcreated),'that.$store.state.userInfo.maincorpid')
    if(Number(this.$store.state.userInfo.corpcreated)>=1652544000){
        this.iscorpcreated = false;
    }else{
        this.iscorpcreated = true;
    }
  },
  computed: {
    pickerStart() {
