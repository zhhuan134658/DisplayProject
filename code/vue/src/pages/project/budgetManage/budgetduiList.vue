 ...
 * @Last Modified time: 2022-08-02 16:53:59
 */
<template>
  <div id="budgetDirect">
    <!-- 直接预算列表 -->
    <div class="main">
      ...
      <div class="mainContent">
        ...
        >
          ...
          <el-table-column label="销项税金额" align="center">
            <el-table-column
              prop="xiaox_money"
              align="left"
              :show-overflow-tooltip="true"
          ...
          <el-table-column label="进项税金额" align="center">
            <el-table-column
              prop="jinx_money"
              align="left"
              :show-overflow-tooltip="true"
          ...
          <el-table-column label="应纳增值税金额" align="center">
            <el-table-column
              prop="zeng_money"
              align="left"
              :show-overflow-tooltip="true"
          ...
          <el-table-column label="附加税金额" align="center">
            <el-table-column
              prop="additional_tax"
              align="left"
              :show-overflow-tooltip="true"
          ...
          <el-table-column label="印花税金额" align="center">
            <el-table-column
              prop="printing_tax"
              align="left"
              :show-overflow-tooltip="true"
          ...
          <el-table-column label="企业所得税金额" align="center">
            <el-table-column
              prop="enterprise_tax"
              align="left"
              :show-overflow-tooltip="true"
          ...
          <el-table-column label="其他税金额" align="center">
            <el-table-column
              prop="other_tax"
              align="left"
              :show-overflow-tooltip="true"
              ...
              <div class="sum_footer xiaoji" ref="sum_xiaoji">
                ...
                <div class="sum_footer_unit over_sum_text">
                  <el-tooltip
                    ...
                    placement="top"
                  >
                    <div style="text-align: left">{{ qt1xiaoji }}</div>
                  </el-tooltip>
                </div>
                <div class="sum_footer_unit over_sum_text">
                  <el-tooltip
                ...
                <div class="sum_footer_unit over_sum_text">
                  <el-tooltip
                    ...
                    placement="top"
                  >
                    <div style="text-align: left">{{ qt1heji }}</div>
                  </el-tooltip>
                </div>
                <div class="sum_footer_unit over_sum_text">
                  <el-tooltip
                    class="item"
                    effect="dark"
                    :content="qt1heji1.toString()"
                    placement="top"
                  >
                    <div style="text-align: left">{{ qt1heji1 }}</div>
                  </el-tooltip>
                </div>
                ...
                <div class="sum_footer_unit over_sum_text">
                  <el-tooltip
                    class="item"
                    effect="dark"
                    :content="qt1heji2.toString()"
                    placement="top"
                  >
                    <div style="text-align: left">{{ qt1heji2 }}</div>
                  </el-tooltip>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    viewDia(row, column, cell, event) {
      ...
      if (column.property == 'xiaox_money') {
        //销项税金额策划
        this.titleData = '销项税金额策划数据来源';

        this.proName = `${'项目名称:' + row.name}`;
        this.totalMoney = `${'销项税金额策划:' + row.xiaox_money}`;
        this.getpierceList('xiaox_money', row, column);
      } else if (column.property == 'jinx_money') {
        //进项税金额
        this.titleData = '进项税金额策划数据来源';

        this.proName = `${'项目名称:' + row.name}`;
        this.totalMoney = `${'进项税金金额:' + row.jinx_money}`;
        this.getpierceList('jinx_money', row, column);
      } else if (column.property == 'additional_tax') {
        //附加税金额
        this.titleData = '附加税金额数据来源';

        this.proName = `${'项目名称:' + row.name}`;
        this.totalMoney = `${'附加税金额:' + row.additional_tax}`;
        this.getpierceList('additional_tax', row, column);
      } else if (column.property == 'enterprise_tax') {
        //企业所得税金额
        this.titleData = '企业所得税金额数据来源';

        this.proName = `${'项目名称:' + row.name}`;
        this.totalMoney = `${'企业所得税金额:' + row.enterprise_tax}`;
        this.getpierceList('enterprise_tax', row, column);
      } else if (column.property == 'other_tax') {
        //其他税金额
        this.titleData = '其他税金额数据来源';

        this.proName = `${'项目名称:' + row.name}`;
        this.totalMoney = `${'其他税金额:' + row.other_tax}`;
        this.getpierceList('other_tax', row, column);
    ...
    getpierceList(biao_shi, row, column) {
      ...
      this.$axios
        .post('/newtao/taxContrastBaoPierce', {
          biao_shi,
    ...
    getList() {
      ...
      this.$axios
        .post('/projectone/taxContrastBao', {
        ...
        .then(res => {
          if (res.data.code == 1) {
            ...
            console.log(this.tpList, 'tpListtpList');
            this.cyxiaoji = res.data.mould_data[1].xj;
            // this.cyheji = res.data.material_jh_total;
            this.csxiaoji = res.data.mould_data[2].xj;
            // this.csheji = res.data.material_payment_total;

            this.lyxiaoji = res.data.mould_data[3].xj;
            // this.lyheji = res.data.service_jh_total;
            this.lsxiaoji = res.data.mould_data[4].xj;
            // this.lsheji = res.data.service_payment_total;

            this.zyxiaoji = res.data.mould_data[5].xj;
            // this.zyheji = res.data.lease_jh_total;
            this.zsxiaoji = res.data.mould_data[6].xj;
            // this.zsheji = res.data.lease_payments_total;

            this.fyxiaoji = res.data.mould_data[7].xj;
            // this.fyheji = res.data.sub_jh_total;
            this.fsxiaoji = res.data.mould_data[8].xj;
            // this.fsheji = res.data.sub_payment_total;
            // this.chsxiaoji = res.data.csf_jh_subtotal;
            // this.chsheji = res.data.csf_jh_total;
            this.qtxiaoji = res.data.mould_data[9].xj;
            this.qtheji = res.data.mould_data[9].xj;

            this.qt1xiaoji = res.data.mould_data[10].xj;
            this.qt1heji = res.data.mould_data[10].xj;
            this.qt1heji1 = res.data.mould_data[11].xj;
            this.qt1heji2 = res.data.mould_data[12].xj;
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/projectone/taxContrastBaoDownload', {
