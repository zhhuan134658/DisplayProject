  ...
  methods: {
    viewDia1(row, column, cell, event) {
      ...
      if (column.property == 'revenue_money') {
        ...
        this.viewVisible = true;
        this.proName = `${'项目名称:' + row.name}`;
        this.totalMoney = `${'支出结算总额:' + event.target.innerText}`;
        this.getpierceList1('1', row, column);
    ...
    getpierceList1(project_name, row, column) {
      ...
      this.$axios
        .post('/newtao/profitContrastBaoPierce', {
        ...
        .then(res => {
          ...
          if (onenewArr.includes(column.property)) {
            ...
            this.tableList = [res.data.con_data, res.data.other_data];
          } else if (column.property == 'settlement') {
            this.tableList = [
    ...
    getList(ntype) {
      ...
      this.$axios
        .post('/projectone/profitContrastBao', {
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/projectone/profitContrastBaoDownload', {
