/* * @Author: NorthWind * @Date: 2020-12-09 10:46:31 * @Last Modified by:
NorthWind * @Last Modified time: 2022-01-13 11:43:46 */
<template>
  <div id="subProgressMoney">
    <!-- 分包进度款列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          :isFooter="isFooter"
          @listenChild="getMult"
          printTableName="sub_progresspay_declaration"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  ...
  methods: {
    ...
    getTitleList() {
      this.$axios
        .post('/spread/showFiledCurrencyList', {
          name: 'sub_progresspay_declaration',
        })
        .then(res => {
          if (res.data.code == 1) {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/project/subProgresspayDeclaration')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    backList() {
      this.$axios
        .post('/project/subProgresspayDeclarationReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/project/subProgresspayDeclarationDownload', {
