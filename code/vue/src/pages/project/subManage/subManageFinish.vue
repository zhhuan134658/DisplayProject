/* * @Author: NorthWind * @Date: 2020-12-09 10:47:04 * @Last Modified by:
NorthWind * @Last Modified time: 2022-01-13 11:44:05 */
<template>
  <div id="subManageFinish">
    <!-- 分包完工结算列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          :isFooter="isFooter"
          @listenChild="getMult"
          printTableName="sub_completion_settlement"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  ...
  methods: {
    ...
    getTitleList() {
      this.$axios
        .post('/spread/showFiledCurrencyList', {
          name: 'sub_completion_settlement',
        })
        .then(res => {
          if (res.data.code == 1) {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/project/subCompletionSettlement')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    backList() {
      this.$axios
        .post('/project/subCompletionSettlementReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/project/subCompletionSettlementDownload', {
