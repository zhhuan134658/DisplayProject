<template>
  <div id="initializeApp">
    ...
    <el-dialog title="提示" :visible.sync="waringVisible" width="30%">
      <div>初始化设置后，数据将会清空，不能恢复，请谨慎操作！</div>
      <div>
        如需初始化,请在输入框内输入"
        <span style="color: #fb7293">确认删除</span> "
      </div>
      <div>
        <el-input placeholder="请输入确认删除" v-model="waRinput"> </el-input>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="waringVisible = false">取 消</el-button>
        <el-button type="primary" @click="waringClick">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    waringClick() {
      if (this.waRinput == '确认删除') {
        this.waringVisible = false;
        this.dialog = true;
        // 同步调用函数
        this.getResult([
          this.getA,
        ...
        this.$message.error('输入错误,请再次确认');
      }
    },
    clear() {
      this.waringVisible = true;
    },
    clear3() {
