<template>
  <div id="qualityAnalysis">
    <div class="main">
      <div class="top">
        <componenTtiles :indexs="9" :pid="pid" :pname="pname"></componenTtiles>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      oneList: [
        ...
        { title: '合同份数(份)', value: '' },
        { title: '关联企业(家)', value: '' },
        { title: '累计入场工人(人)', value: '' },
        { title: '结算金额(元)', value: '' },
        { title: '已支付金额(元)', value: '' },
        { title: '待支付金额(元)', value: '' },
        { title: '零星用工支付金额(元)', value: '' },
        { title: '自有工人支付金额(元)', value: '' },
        { title: '总支付金额(元)', value: '' },
      ],
      changListTwo: [
        { title: '按时间排序' },
  ...
  methods: {
    ...
    getList(ntype) {
      this.$axios
        .post('/bulletin/labourContractAnalysisUnit', {
      ...
      this.$axios
        ...
        .then(res => {
          if (res.data.code == 1) {
            ...
            console.log(this.tpList, this.titleList, 'tpListtpListtpList');
            this.footerNumList = [];
    ...
    getLInfo() {
      ...
      _this.$axios
        .post('/bulletin/labourInfo', {
          project_id: _this.pid,
          project_name: _this.pname,
        })
        .then(res => {
          if (res.data.code == 1) {
            _this.oneList[0].value = res.data.data.ht_money;
            _this.oneList[1].value = res.data.data.ht_number;
            _this.oneList[2].value = res.data.data.qy_number;
            _this.oneList[3].value = res.data.data.worker_number;
            _this.oneList[4].value = res.data.data.sett_money;
            _this.oneList[5].value = res.data.data.paid_money;
            _this.oneList[6].value = res.data.data.no_paid_money;
            _this.oneList[7].value = res.data.data.spo_money;
            _this.oneList[8].value = res.data.data.xmgz_money;
            _this.oneList[9].value = res.data.data.zong_paid_money;
          }
        })
        .catch(function (error) {
          console.log(error);
