/* * @Author: NorthWind * @Date: 2020-12-10 14:32:15 * @Last Modified by:
NorthWind * @Last Modified time: 2022-01-13 14:00:25 */
<template>
  <div id="equipmentMaintain">
    <!-- 设备维保列表 -->
    <div class="main">
      <!-- 搜索条件 -->
      <commonSearch
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          :isFooter="isFooter"
          @listenChild="getMult"
          printTableName="equipment_maintenance"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  ...
  methods: {
    ...
    getTitleList() {
      this.$axios
        .post('/spread/showFiledCurrencyList', {
          name: 'other_expenditure_settlement',
        })
        .then(res => {
          if (res.data.code == 1) {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post('/projecttwo/otherExpenditureSettlementAdd')
        .then(res => {
          if (res.data.code == 1) {
            let newUrl =
    ...
    getList(ntype) {
      this.$axios
        .post('/projecttwo/otherExpenditureSettlementList', {
    ...
    deleteList() {
      if (this.multSelectData.length > 0) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          .then(() => {
            this.$axios
              .post('/projecttwo/otherExpenditureSettlementDel', {
    ...
    backList() {
      this.$axios
        .post('/project/equipmentMaintenanceReduction')
        .then(res => {
          if (res.data.code == 1) {
            this.getList();
            this.$message({
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post('/projecttwo/otherExpenditureSettlementload', {
