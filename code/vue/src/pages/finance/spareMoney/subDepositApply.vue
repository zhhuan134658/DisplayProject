      ...
      <div class="selectorGroups">
        <div style="padding: 10px">
          <el-tabs v-model="is_complete" @tab-click="handleClick">
            <el-tab-pane label="投标保证金支出" name="1"> </el-tab-pane>
            <el-tab-pane label="履约保证金支出" name="2"> </el-tab-pane>
            <el-tab-pane label="劳务保证金退回" name="3"></el-tab-pane>
            <el-tab-pane label="分包保证金退回" name="4"></el-tab-pane>
            <el-tab-pane
              label="老数据"
              name="5"
      ...
      <div class="mainContent">
        ...
        <!-- <div class="selectorGroups">
          <el-radio-group v-model="is_complete" @change="getGant">
            <el-radio-button label="1">投标保证金支出</el-radio-button>
            <el-radio-button label="2">履约保证金支出</el-radio-button>
            <el-radio-button label="3">劳务保证金退回</el-radio-button>
            <el-radio-button label="4">分包保证金退回</el-radio-button>
        ...
        <commonTable
          ...
          :codeShow="codeShow"
          :types="biaoName()"
          :isdele8="is_complete != '5' ? true : false"
          printTableName="bond_payment_register"
        ></commonTable>
        <div class="page">
          <el-pagination
...
export default {
  ...
  methods: {
    ...
    addName() {
      let names = '';
      if (this.is_complete == '1') {
        names = '/project/bidBondApply';
      } else if (this.is_complete == '2') {
        names = '/finance/bondpayapply';
      } else if (this.is_complete == '3') {
        names = '/finance/services_bond_return';
      } else if (this.is_complete == '4') {
        names = '/finance/sub_bond_return';
      } else {
        names = '';
      }
      return names;
    ...
    biaoPost() {
      ...
      if (this.is_complete == '1') {
        newapi.Post = '/project/bidBondApplyList';
        apiData = { page: this.currentPage, number: this.pagesize };
      } else if (this.is_complete == '2') {
        newapi.Post = '/finance/bondpayapplylist';
      } else if (this.is_complete == '3') {
        newapi.Post = '/finance/services_bond_returnlist';
      } else if (this.is_complete == '4') {
        newapi.Post = '/finance/sub_bond_returnlist';
      } else {
        newapi.Post = '/finance/bond_payment_registerlist';
      }
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (res.data.code == 1) {
            ...
            if (this.isFooter == 1) {
              this.newFooter = res.data[0].mould_data.mould_data;
              this.titleList.map(item => {
                this.newFooter.forEach(element => {
                  if (element.value == item.value) {
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        ...
        // _this.$axios
        //   .post('/finance/bond_payment_registerdc', {
        ...
        let newApi =
          _this.addType() == 1
            ? '/project/bidBondApplyDownload'
            : _this.addType() == 2
            ? '/finance/bondpayapplydc'
            : _this.addType() == 3
            ? '/finance/services_bond_returndc'
            : '/finance/sub_bond_returndc';
        _this.$axios
          .post(newApi, {
            id: _this.multSelectData,
          })
