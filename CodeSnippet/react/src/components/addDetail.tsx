    ...
    const wrappedFinish = (values: any) => {
        ...
        if (values['type']) {
            for (let i = values['type'].length; i > 0; i--) {
                if (values['type'][i - 1]) {
                    parsedValues['type'] = JSON.parse(values['type'][i - 1])[
                        'key'
                    ];
                    break;
                }
